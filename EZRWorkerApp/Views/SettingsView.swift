// EZRWorkerApp/Views/SettingsView.swift
        ...
        Form {
            ...
            Section(L10n.k("views.settings_view.text_ffa993d5", fallback: "虾塘显示")) {
                ...
                if showCurrentAdminInPool {
                    ...
                    Text(L10n.k("views.settings_view.ezrworker_account_delete_account", fallback: "安全保障：EZRWorker 已对管理员账户禁用部分高风险动作（如重置/删除），但仍建议日常只使用标准用户账户。"))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                } else {
    ...
    var body: some View {
        Section(L10n.k("views.settings_view.privacy_security", fallback: "隐私与安全")) {
            if lockStore.isEnabled {
                ...
                Text(L10n.k("views.settings_view.admin_password_ezrworker", fallback: "启用锁定后，每次开机或系统屏幕锁定后需输入管理密码才能使用 EZRWorker。"))
                    .font(.caption)
                    .foregroundStyle(.secondary)
            } else {
                LabeledContent(L10n.k("views.settings_view.app_lock", fallback: "App 锁定")) {
...
struct HelperLogTab: View {
    ...
    private let logPath = "/tmp/ezrworker-helper.log"
