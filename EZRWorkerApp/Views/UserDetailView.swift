// EZRWorkerApp/Views/UserDetailView.swift
...
enum QuickFileTransferService {
    static let destinationRelativePath = ".openclaw/ezrworker_upload"
        ...
        VStack(alignment: .leading, spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                if !helperClient.isConnected {
                    Text(L10n.k("user.detail.auto.helper_ezrworker", fallback: "Helper 未连接，请先安装 EZRWorker 系统服务"))
                        .font(.caption)
                        .foregroundStyle(.orange)
                }
    ...
    private func doApplyConfig(apiKey: String) async {
        ...
        do {
            ...
            let (noop, _) = try await gatewayHub.configPatch(
                ...
                patch: mergedPatch,
                baseHash: baseHash,
                note: "EZRWorker: apply \(selectedProvider.title) config"
            )
