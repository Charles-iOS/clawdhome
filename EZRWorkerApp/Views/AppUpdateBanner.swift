        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            HStack(spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text(L10n.k("auto.app_update_banner.ezrworker_version", fallback: "EZRWorker 有新版本"))
                        .font(.headline)
                    HStack(spacing: 6) {
            ...
            if updater.isAwaitingAppRelaunch {
                VStack(spacing: 10) {
                    ProgressView()
                        .controlSize(.small)
                        .tint(.orange)
                    Text(L10n.k("auto.app_update_banner.waiting_for_install", fallback: "安装器已打开。完成安装后，EZRWorker 会自动重新打开。"))
                        .font(.callout)
                        .foregroundStyle(.secondary)
                        .frame(maxWidth: .infinity, alignment: .leading)
                }
