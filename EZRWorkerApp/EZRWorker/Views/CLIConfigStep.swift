// EZRWorkerApp/Views/CLIConfigStep.swift
...
struct CLIConfigStep: View {
    ...
    private func openTerminal() -> Bool {
        let terminalTitle = L10n.f("views.cliconfig_step.terminal_title", fallback: "=== EZRWorker: 配置用户 %@ ===", user.username)
        let passwordHint = L10n.f("views.cliconfig_step.password_hint", fallback: "提示：接下来请输入 %@ 的 Mac 登录密码", NSUserName())
        let successLine = L10n.k("views.cliconfig_step.terminal_success", fallback: "配置完成。请返回 EZRWorker 点击「已完成配置」。")
