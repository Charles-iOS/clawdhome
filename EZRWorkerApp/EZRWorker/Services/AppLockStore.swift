// EZRWorkerApp/Services/AppLockStore.swift
...
final class AppLockStore {
    ...
    init() {
        ...
        let enabled = UserDefaults.standard.object(forKey: Self.enabledDefaultsKey) as? Bool ?? false
        isEnabled   = enabled
        isLocked    = enabled

        // biometric 设置存在 UserDefaults（也不触发弹窗）
        isBiometricEnabled = UserDefaults.standard.object(forKey: Self.enabledDefaultsKey + ".biometric") as? Bool ?? false
    }
    ...
    func unlockWithBiometrics() async {
        ...
        do {
            let success = try await context.evaluatePolicy(
                ...
                localizedReason: L10n.k("services.app_lock_store.unlock_ezrworker", fallback: "解锁 EZRWorker")
            )
