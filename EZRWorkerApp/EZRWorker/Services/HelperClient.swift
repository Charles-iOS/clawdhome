// EZRWorker/Services/HelperClient.swift
// 封装 EZRWorker.app 与 EZRWorkerHelper 之间的 XPC 连接
...
final class HelperClient {
    ...
    private func makeConnection(label: String) -> NSXPCConnection {
        ...
        conn.remoteObjectInterface = NSXPCInterface(with: EZRWorkerHelperProtocol.self)
    ...
    private func verifyConnection() {
        ...
        }) as? any EZRWorkerHelperProtocol else {
            isConnected = false
            return
        }
    ...
    private func proxyWithLogging(_ conn: NSXPCConnection?) -> (any EZRWorkerHelperProtocol)? {
        ...
        } as? any EZRWorkerHelperProtocol
    }

    private var controlProxy: (any EZRWorkerHelperProtocol)? {
        proxyWithLogging(controlConnection)
    }

    private var dashboardProxy: (any EZRWorkerHelperProtocol)? {
        proxyWithLogging(dashboardConnection)
    }

    private var installProxy: (any EZRWorkerHelperProtocol)? {
        proxyWithLogging(installConnection)
    }

    private var fileProxy: (any EZRWorkerHelperProtocol)? {
        proxyWithLogging(fileConnection)
    }

    private var processProxy: (any EZRWorkerHelperProtocol)? {
        proxyWithLogging(processConnection)
    }

    private var personaReadProxy: (any EZRWorkerHelperProtocol)? {
    ...
    /// 设置 Gateway 开机自启（写标志文件到 /var/lib/ezrworker/）
    ...
    /// 安装或升级指定用户的 openclaw（输出实时写入 /tmp/ezrworker-init-<username>.log）
    ...
    /// 安装 Node.js（输出实时写入 /tmp/ezrworker-init-<username>.log）
    ...
    /// 保存向导进度 JSON（写入 /var/lib/ezrworker/<username>-init.json）
    ...
    /// 读取 /var/log/ezrworker/ 下的系统审计日志（name: "gateway"）
...
enum HelperError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .notConnected:              return L10n.k("services.helper_client.helper_ezrworker", fallback: "Helper 未连接，请确认 EZRWorker 已正确安装")
