...
## Summary
本次整理以“`主线 + 仍会被打开的旧窗口链路`”为范围，把当前实际在用的 `EZRWorker` App 侧代码按技术层归拢到现有 `EZRWorkerApp/EZRWorker` 目录下；不删除旧文件，也不处理 `Shared` / `EZRWorkerHelper` 这类跨 target 代码。

当前已确认的事实：
- `EZRWorkerApp` 真实入口已切到 `MainView`。
- `EZRWorker` 目录目前为空。
...
- 把 App 主线文件移入 `EZRWorker/App`：
  ...
  - `MainView.swift`
  - `Views/Sidebar/*`
  - 保留 `EZRWorkerApp.swift` 里现有旧窗口包装类型在同文件内，不额外拆分，避免无意义访问级别调整。
...
- 把 App 侧业务层一并移入 `EZRWorker/Services`、`EZRWorker/Models`、`EZRWorker/Utils`：
  - `EZRWorkerApp/Services/*` 整体迁入 `EZRWorker/Services`
  - `EZRWorkerApp/Models/*` 整体迁入 `EZRWorker/Models`
  - `EZRWorkerApp/Utils/*` 整体迁入 `EZRWorker/Utils`
  - 原因：主线与旧窗口混用这些服务/模型，整体迁移比只搬一部分更稳，避免新旧目录交叉引用继续恶化。

- 把 App 自有资源迁入 `EZRWorker/Resources`：
  - `EZRWorkerApp/Resources/*`
  - 不移动 `Info.plist`、`Assets.xcassets`、`Stable.xcstrings`、`Localization/*`、`AppIcon.icns`，这些继续留在 `EZRWorkerApp/` 根层作为包级资源。
...
- 更新 `EZRWorker.xcodeproj/project.pbxproj`：
...
## Test Plan
- 构建验证：
  - `xcodebuild` 构建 `EZRWorker` target，确认路径迁移后项目仍可编译。
  - 如有独立 helper 依赖，一并确认 `EZRWorkerHelper` 未受 `Shared` 保持原位策略影响。
...
## Assumptions
- 使用现有目录名 `EZRWorkerApp/EZRWorker`，不按你消息里的拼写另建 `EZRWroker`。
- “把用到的文件移动到 EZRWorker 下”默认解释为：`EZRWorker` App target 内当前主线和旧窗口链路所依赖的源码迁入；跨 target 的 `Shared` 与 `EZRWorkerHelper` 不迁。
