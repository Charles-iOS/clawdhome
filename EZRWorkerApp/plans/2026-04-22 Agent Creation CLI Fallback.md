...
## 目标
把 EZRWorker 的“新建智能体”流程改成：
- 首选调用 OpenClaw 原生命令 `openclaw agents add <id>`；
- 若 CLI 路径失败，自动回退到当前实现（`config.patch + workspace 初始化`）；
- 仅影响后续新建，不改动已有不完整工作区。

## 变更点
- 在 [EZRWorkerApp/Services/Stores/AgentStore.swift](../EZRWorker/Services/Stores/AgentStore.swift) 中重构 `addAgent(...)`：
  - 拆分为 `tryAddAgentViaCLI(...)` 与 `addAgentViaPatchFallback(...)` 两段逻辑；
  - 主路径先走 CLI，失败时记录日志并回退。
- 复用/扩展 [EZRWorkerApp/Services/Gateway/GatewayProcessManager.swift](../EZRWorker/Services/Gateway/GatewayProcessManager.swift) 本地命令执行能力：
  - 增加一个 `agents add` 专用包装（参数构造、输出收集、错误返回）；
  - 保持执行环境与现有 `pairing` 命令一致，避免路径/权限差异。
- 在 [EZRWorkerApp/Views/Agent/CreateAgentSheet.swift](../EZRWorker/Views/Agent/CreateAgentSheet.swift) 维持现有 UI 交互：
  - 不改表单字段；
