...
| 组件 | 路径 | 用途 |
|------|------|------|
| `ChannelType` | [ChannelType.swift](EZRWorkerApp/Models/ChannelType.swift) | 5 渠道元数据 |
| `ChannelBotConfigSheet` | [ChannelBotConfigSheet.swift](EZRWorkerApp/Views/Capabilities/ChannelBotConfigSheet.swift) | 凭据表单（钉钉/飞书/Telegram/Discord） |
| `FeishuChannelOnboardingSheet` | [FeishuChannelOnboardingSheet.swift](EZRWorkerApp/Views/ChannelOnboarding/FeishuChannelOnboardingSheet.swift) | QR 扫码 PTY 终端（飞书/微信） |
| `ChannelOnboardingFlow` | 同上 | 飞书/微信 CLI 命令参数 |
| `AgentStore` | [AgentStore.swift](EZRWorkerApp/Services/Stores/AgentStore.swift) | `addBinding`/`removeBinding` CRUD |

---

## 改动一：消息渠道入口（渠道 → 选智能体）

**文件**: [ChannelView.swift](EZRWorkerApp/Views/Capabilities/ChannelView.swift)
...
**文件**: [AgentBindingsView.swift](EZRWorkerApp/Views/Agent/AgentBindingsView.swift)
...
**文件**: [AgentBinding.swift](EZRWorkerApp/Models/AgentBinding.swift)
...
**文件**: [AgentStore.swift](EZRWorkerApp/Services/Stores/AgentStore.swift)

`private var username` → `private(set) var username`（供 QR onboarding 获取 Shrimp 用户名）。

### 3.3 ChannelType 添加 SwiftUI Color

**文件**: [ChannelType.swift](EZRWorkerApp/Models/ChannelType.swift)
