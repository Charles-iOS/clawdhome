---
name: EZRWorker Agent Config Redesign
overview: 将 EZRWorker 从多用户 OpenClaw 管理工具重构为单实例智能体配置管理面板：去掉 XPC 特权 Helper，pkg 内置 Node.js + OpenClaw 离线安装，App 直接管理本地 Gateway 进程，UI 转为智能体配置 + 能力管理界面。用户通过飞书/微信等渠道与智能体交互，App 负责环境部署和后台配置。
todos:
  - id: phase1-infra
...
# EZRWorker 架构重设计：单实例智能体配置管理面板

## 一、产品定位

**不是聊天应用**。用户通过外部渠道（飞书/微信等）与智能体对话，EZRWorker 的职责是：
...
flowchart LR
  subgraph current [Current: Multi-User Manager]
    App1[EZRWorker.app] -->|XPC| Helper1[root Helper daemon]
  ...
  subgraph target [Target: Single-Instance Config Panel]
    App2[EZRWorker.app] -->|"Process()"| GW3[OpenClaw Gateway]
...
3. 可选：注册 LaunchAgent `~/Library/LaunchAgents/ai.ezrworker.gateway.plist`（开机自启）
...
**GatewayClient** (复用 [GatewayClient.swift](EZRWorkerApp/Services/GatewayClient.swift))
- WebSocket JSON-RPC 管理通道
- 去掉多用户参数，固定连接 `ws://127.0.0.1:18100/`
- 保留现有的 `connect` 握手、`config.*`、`models.*`、`cron.*`、`skills.*` 方法

**GatewayService** (由 [GatewayHub.swift](EZRWorkerApp/Services/GatewayHub.swift) 简化)
- 从多用户 Hub 简化为单实例服务
...
- [GatewayClient.swift](EZRWorkerApp/Services/GatewayClient.swift) — 去掉多用户参数，固定单实例
- [GatewayHub.swift](EZRWorkerApp/Services/GatewayHub.swift) → 简化为 `GatewayService`
- [GatewayCronStore](EZRWorkerApp/Services/GatewayCronStore.swift) — 去掉 username 参数
- [GatewaySkillsStore](EZRWorkerApp/Services/GatewaySkillsStore.swift) — 去掉 username 参数
- [ProviderKeychainStore](EZRWorkerApp/Services/ProviderKeychainStore.swift) — 直接复用
- [DeviceIdentity](EZRWorkerApp/Services/DeviceIdentity.swift) — 直接复用
- `CronTabView` / `SkillsTabView` 内的核心 UI 逻辑 — 从 [UserDetailView.swift](EZRWorkerApp/Views/UserDetailView.swift) 中提取独立
- `FeishuChannelOnboardingSheet` 逻辑 — 改造为独立渠道配置页
- [SettingsView.swift](EZRWorkerApp/Views/SettingsView.swift) — 复用大部分
- `roles.html` 中的 24 个角色数据 → 提取为 `PresetAgents.json`
- 部分 ModelManager Views — 复用模型配置 UI

### 删除

- **EZRWorkerHelper/** 整个 target — 不再需要特权 daemon
- **Shared/HelperProtocol.swift** — XPC 协议
- **Shared/ 中的 XPC 相关模型** — `DashboardModels` 等服务器监控模型
- [HelperClient.swift](EZRWorkerApp/Services/HelperClient.swift) — XPC 客户端
- [ShrimpPool](EZRWorkerApp/Services/ShrimpPool.swift) / ManagedUser — 多用户管理
...
EZRWorkerApp/
├── App/
│   └── EZRWorkerApp.swift          # @main 入口，纯粹的环境注入
├── Models/
...
- 所有 Service/Store 通过 `@Environment` 注入，在 `EZRWorkerApp.swift` 统一创建
...
- `App/EZRWorkerApp.swift` — 重写入口，注入新的 Service 集合
