// EZRWorkerHelper/Operations/InstallManager.swift
...
enum InstallError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .xcodeCommandLineToolsMissing:
            return "检测到缺少 Xcode Command Line Tools。请先在 EZRWorker 的「开发环境修复」中点击“安装开发工具”，完成后重试。若需终端方式，可执行 `xcode-select --install`。"
        case .xcodeLicenseNotAccepted:
            return "检测到 Xcode license 未接受。请先在 EZRWorker 的「开发环境修复」中点击“同意 Xcode 许可”，完成后重试。若需终端方式，可执行 `sudo xcodebuild -license accept`。"
        case .xcodeToolchainNotReady(let details):
            return "检测到 Xcode/CLT 工具链未就绪。请先在 EZRWorker 的「开发环境修复」中完成修复后重试。\n\(details)"
        }
    }
}
