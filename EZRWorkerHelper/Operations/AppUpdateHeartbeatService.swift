...
final class AppUpdateHeartbeatService {
    ...
    private let queue = DispatchQueue(label: "ai.ezrworker.helper.app-update-heartbeat", qos: .utility)
    ...
    private func checkNow() async {
        ...
        let clientHeader = buildClientHeader(appVersion: appVersion, systemLanguage: systemLanguage)
        request.setValue(clientHeader, forHTTPHeaderField: "X-EZRWorker-Client")
    ...
    private func installedAppVersion() -> String {
        let candidates = [
            "/Applications/EZRWorker.app/Contents/Info.plist",
            Bundle.main.bundleURL
                .deletingLastPathComponent()
                .appendingPathComponent("EZRWorker.app/Contents/Info.plist")
                .path
        ]
    ...
    private func buildUserAgent(appVersion: String, systemLanguage: String) -> String {
        "EZRWorker/\(appVersion) (macOS \(Self.systemVersionString()); \(Self.cpuArchitecture()); \(Self.cpuModel()); \(Self.physicalMemoryString()); \(systemLanguage))"
    }
