// EZRWorkerHelper/main.swift
// EZRWorkerHelper LaunchDaemon — root 权限常驻服务
// 接受来自 EZRWorker.app 的 XPC 请求，代理执行跨用户操作
...
private let helperLogPath = "/tmp/ezrworker-helper.log"
private let helperLogQueue = DispatchQueue(label: "ai.ezrworker.helper.log", qos: .utility)
...
private let helperDebugFlagPath = "/var/lib/ezrworker/helper-debug-logging-enabled"
private let helperDebugLock = NSLock()
private var helperDebugEnabled: Bool = {
    let env = (
        ProcessInfo.processInfo.environment["EZRWORKER_HELPER_DEBUG"] ??
        ProcessInfo.processInfo.environment["CLAWDHOME_HELPER_DEBUG"] ??
        ""
    ).lowercased()
...
private enum GatewayIntentionalStopStore {
    private static let baseDir = "/var/lib/ezrworker"
...
private func gatewayAutostartGloballyEnabled() -> Bool {
    !FileManager.default.fileExists(atPath: "/var/lib/ezrworker/gateway-autostart-disabled")
}

private func userGatewayAutostartEnabled(username: String) -> Bool {
    !FileManager.default.fileExists(atPath: "/var/lib/ezrworker/\(username)-autostart-disabled")
}
...
private final class GatewayWatchdog {
    ...
    private let queue = DispatchQueue(label: "ai.ezrworker.helper.gateway-watchdog", qos: .background)
...
private final class MaintenanceTerminalSession {
    ...
    private static func ensureNpxShimDirectory(username: String) throws -> String {
        let shimDir = "/tmp/ezrworker-maintenance-shims/\(username)"
...
final class EZRWorkerHelperImpl: NSObject, EZRWorkerHelperProtocol {
    ...
    func createUser(username: String, fullName: String, password: String,
        ...
        do {
            ...
            let autostartDisabledPath = EZRWorkerHelperImpl.userAutostartDisabledPath(username: username)
            try? FileManager.default.createDirectory(
                atPath: "/var/lib/ezrworker",
                withIntermediateDirectories: true,
                attributes: nil
    ...
    private static let autostartDisabledPath = "/var/lib/ezrworker/gateway-autostart-disabled"

    func setGatewayAutostart(enabled: Bool, withReply reply: @escaping (Bool, String?) -> Void) {
        helperLog("全局自启 \(enabled)")
        let path = EZRWorkerHelperImpl.autostartDisabledPath
        if enabled {
            try? FileManager.default.removeItem(atPath: path)
        } else {
            try? FileManager.default.createDirectory(
                atPath: "/var/lib/ezrworker", withIntermediateDirectories: true, attributes: nil)
            FileManager.default.createFile(atPath: path, contents: nil)
        }
        reply(true, nil)
    }
    ...
    func getGatewayAutostart(withReply reply: @escaping (Bool) -> Void) {
        reply(!FileManager.default.fileExists(atPath: EZRWorkerHelperImpl.autostartDisabledPath))
    }
    ...
    private static func userAutostartDisabledPath(username: String) -> String {
        "/var/lib/ezrworker/\(username)-autostart-disabled"
    }

    func setUserAutostart(username: String, enabled: Bool,
                          withReply reply: @escaping (Bool, String?) -> Void) {
        helperLog("用户自启 @\(username) \(enabled)")
        let path = EZRWorkerHelperImpl.userAutostartDisabledPath(username: username)
        if enabled {
            try? FileManager.default.removeItem(atPath: path)
        } else {
            try? FileManager.default.createDirectory(
    ...
    func getUserAutostart(username: String, withReply reply: @escaping (Bool) -> Void) {
        reply(!FileManager.default.fileExists(
            atPath: EZRWorkerHelperImpl.userAutostartDisabledPath(username: username)))
    }
    ...
    func repairHomebrewPermission(username: String, withReply reply: @escaping (Bool, String?) -> Void) {
        ...
        let sharedCacheRoot = "/Users/Shared/EZRWorkerApp/cache"
        let homebrewCacheDir = "\(sharedCacheRoot)/homebrew"
        let installScript = """
        set -e
        BREW_ROOT="$HOME/.brew"
        CACHE_DIR="/Users/Shared/EZRWorkerApp/cache/homebrew"
    ...
    func cancelInit(username: String, withReply reply: @escaping (Bool) -> Void) {
        let logPath = "/tmp/ezrworker-init-\(username).log"
    ...
    func loadInitState(username: String,
                       withReply reply: @escaping (String) -> Void) {
        let path = "/var/lib/ezrworker/\(username)-init.json"
    ...
    func getShrimpNetworkPolicy(username: String, withReply reply: @escaping (String?) -> Void) {
        ...
        let path = "/var/lib/ezrworker/\(username)-netpolicy.json"
    ...
    func setShrimpNetworkPolicy(username: String, policyJSON: String, withReply reply: @escaping (Bool, String?) -> Void) {
        ...
        do {
            try FileManager.default.createDirectory(
                atPath: "/var/lib/ezrworker", withIntermediateDirectories: true,
                attributes: [.posixPermissions: 0o755])
    ...
    func getGlobalNetworkConfig(withReply reply: @escaping (String?) -> Void) {
        let json = try? String(contentsOfFile: "/var/lib/ezrworker/global-netconfig.json", encoding: .utf8)
        reply(json)
    }

    func setGlobalNetworkConfig(configJSON: String, withReply reply: @escaping (Bool, String?) -> Void) {
        let path = "/var/lib/ezrworker/global-netconfig.json"
        do {
            try FileManager.default.createDirectory(
                atPath: "/var/lib/ezrworker", withIntermediateDirectories: true,
                attributes: [.posixPermissions: 0o755])
    ...
    private func rewriteProxyManagedBlock(
        ...
        if enabled, !proxyURL.isEmpty {
            ...
            let block = """
                \(start)
                # Managed by EZRWorker. Do not edit manually.
    ...
    func readSystemLog(name: String, withReply reply: @escaping (Data?, String?) -> Void) {
        ...
        let path = "/var/log/ezrworker/\(name).log"
    ...
    func syncSecrets(username: String, secretsJSON: String, authProfilesJSON: String,
        ...
        do {
            guard let pw = getpwnam(username) else {
                throw NSError(domain: "EZRWorkerHelper", code: -1,
            ...
            guard let secretsData = secretsJSON.data(using: .utf8) else {
                throw NSError(domain: "EZRWorkerHelper", code: -2,
            ...
            guard let authProfilesData = authProfilesJSON.data(using: .utf8) else {
                throw NSError(domain: "EZRWorkerHelper", code: -3,
...
final class ListenerDelegate: NSObject, NSXPCListenerDelegate {
    func listener(_ listener: NSXPCListener,
        ...
        connection.exportedInterface = NSXPCInterface(with: EZRWorkerHelperProtocol.self)
        connection.exportedObject = EZRWorkerHelperImpl()
        connection.resume()
    ...
    ///   2. 代码签名为 EZRWorker.app（DEBUG 跳过，Release 用 auditToken 强制校验）
...
// 若 /var/lib/ezrworker/gateway-autostart-disabled 存在则跳过（用户在设置中关闭了自启）
