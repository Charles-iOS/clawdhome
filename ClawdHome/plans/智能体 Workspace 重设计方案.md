...
**目标**：将 ClawdHome 的智能体管理从 prompt 模板升级为 OpenClaw 原生多智能体管理，支持：
...
#### 1.1 重写 `Agent` 模型
**修改 `ClawdHome/Models/Agent.swift`**
...
**新建 `ClawdHome/Models/AgentStatus.swift`**
...
**新建 `ClawdHome/Models/AgentBinding.swift`**
...
**`ClawdHome/Models/AgentCategory.swift`** 不变。

### Phase 2: 服务层重设计

#### 2.1 重写 `AgentStore`
**修改 `ClawdHome/Services/Stores/AgentStore.swift`**
...
**新建 `ClawdHome/Services/AgentWorkspaceManager.swift`**
...
**修改 `ClawdHome/Views/Agent/AgentGridView.swift`**
...
**修改 `ClawdHome/Views/Agent/AgentCardView.swift`**
...
**新建 `ClawdHome/Views/Agent/AgentWorkspaceView.swift`**
...
**新建 `ClawdHome/Views/Agent/AgentBindingsView.swift`**
...
**新建 `ClawdHome/Views/Agent/AgentSessionsView.swift`**
...
**修改 `ClawdHome/Views/Agent/AgentEditorView.swift`**
...
**修改 `ClawdHome/Views/Agent/CreateAgentSheet.swift`**
...
#### 3.8 导航更新
**修改 `ClawdHome/Views/MainView.swift`**
...
**修改 `ClawdHome/Views/PersonaDefView.swift` 中 `PersonaFile.relPath`**
...
**修改 `ClawdHome/ClawdHomeApp.swift`**
...
| 文件 | 用途 |
|------|------|
| `ClawdHome/Models/AgentStatus.swift` | 智能体运行状态枚举 |
| `ClawdHome/Models/AgentBinding.swift` | 渠道绑定模型（对应 OpenClaw bindings） |
| `ClawdHome/Services/AgentWorkspaceManager.swift` | Workspace 文件操作服务（通过 HelperClient XPC） |
| `ClawdHome/Views/Agent/AgentWorkspaceView.swift` | 分栏式 workspace 编辑器 + bindings + sessions + settings |
| `ClawdHome/Views/Agent/AgentBindingsView.swift` | 渠道绑定管理子视图 |
| `ClawdHome/Views/Agent/AgentSessionsView.swift` | 对话历史浏览子视图 |

### 修改文件（8 个）
| 文件 | 改动 |
|------|------|
| `ClawdHome/Models/Agent.swift` | 移除 systemPrompt/identity/userTemplate/isActive，新增 workspace/agentDir/isDefault/status/boundBindings/sessionCount |
| `ClawdHome/Services/Stores/AgentStore.swift` | 改为从 gateway config 驱动，CRUD 通过 configPatch，支持多智能体 + bindings 管理 |
| `ClawdHome/Views/Agent/AgentGridView.swift` | NavigationLink 到 workspace、状态徽章、预置模板库区域、右键菜单 |
| `ClawdHome/Views/Agent/AgentCardView.swift` | 状态指示器、渠道图标、session 统计 |
| `ClawdHome/Views/Agent/AgentEditorView.swift` | 简化为元数据编辑（移除 prompt/identity/激活按钮） |
| `ClawdHome/Views/Agent/CreateAgentSheet.swift` | 两种创建方式（从模板 / 空白），agentId 输入，workspace 初始化 |
| `ClawdHome/Views/MainView.swift` | agents 区域用 NavigationStack 包裹，支持 push 到 workspace |
| `ClawdHome/Views/PersonaDefView.swift` | PersonaFile.relPath 支持 agentId 参数 |
| `ClawdHome/ClawdHomeApp.swift` | 注入 AgentWorkspaceManager，更新 bootstrap |

### 不修改
| 文件 | 原因 |
|------|------|
| `Shared/HelperProtocol.swift` | 现有 readFile/writeFile/createDirectory/commitPersonaFile 等 XPC 方法完全够用 |
| `ClawdHome/Services/HelperClient.swift` | 复用 fileConnection XPC 通道 |
| `ClawdHome/Services/Gateway/GatewayService.swift` | configGetFull/configPatch API 不变 |
| `ClawdHome/Models/AgentCategory.swift` | 分类体系不变 |
