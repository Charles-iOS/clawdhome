...
todos:
  ...
  - id: phase3-profiles
    ...
    status: pending
  - id: phase4-migration
    content: "Phase 4: 实现 ClawdHome -> EZRWorker 数据迁移和老单实例迁移选择流"
    status: pending
  - id: phase5-cleanup
...
3. **品牌与工程命名不统一**
   - 仓库/project 仍叫 `ClawdHome`
   - 运行时产品名已是 `EZRWorker`
   - App Support、Keychain、Mach service、pkg、文档、脚本仍大量保留 `clawdhome` 标识
...
- 现有 `ClawdHome/EZRWorker/*` 主线代码提升到 `EZRWorkerApp/*`
...
- `BrandMigrationManager`

首次打开 App 时执行：

若：

- `~/Library/Application Support/ClawdHome/` 存在
- 且 `~/Library/Application Support/EZRWorker/` 不存在
...
若新 service 无数据，则把旧 service 迁移到新 service：

- `ai.clawdhome.mac` -> `ai.ezrworker.mac`
- `ai.clawdhome.mac.accounts` -> `ai.ezrworker.mac.accounts`
- `ai.clawdhome.mac.user-pw` -> `ai.ezrworker.mac.user-pw`
- `ai.clawdhome.mac.applock` -> `ai.ezrworker.mac.applock`
...
- 构建产物为：
  ...
  - `EZRWorkerHelper`
  - `EZRWorkerSupervisor`
- bundle id、Mach service、User-Agent、pkg 名称不再含 `clawdhome`
...
- `Application Support/ClawdHome` 与旧 Keychain service 能被正确迁入 `EZRWorker`
